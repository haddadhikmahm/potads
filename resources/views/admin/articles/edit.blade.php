...
<div class="max-w-4xl bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" class="w-full rounded-xl border-gray-200 focus:border-potads-blue focus:ring-potads-blue font-bold text-xl transition" placeholder="Masukkan judul..." value="{{ old('title', $article->title) }}" required>
        ...
        <div class="grid grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="category" id="category" class="w-full rounded-xl border-gray-200 focus:border-potads-blue focus:ring-potads-blue transition" value="{{ old('category', $article->category) }}">
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full rounded-xl border-gray-200 focus:border-potads-blue focus:ring-potads-blue transition">
                    <option value="draft" {{ old('status', $article->status) == 'draft' ? 'selected' : '' }}>Draf (Draft)</option>
        ...
        <div>
            ...
            <textarea name="content" id="content" rows="12" class="w-full rounded-xl border-gray-200 focus:border-potads-blue focus:ring-potads-blue transition leading-relaxed">{{ old('content', $article->content) }}</textarea>
            @error('content') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2">Gambar Utama Saat Ini</label>
            @if($article->image)
                <img src="{{ asset('storage/' . $article->image) }}" class="w-32 h-20 object-cover rounded-lg mb-4 shadow-sm">
            @endif
